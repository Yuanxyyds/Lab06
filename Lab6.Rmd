...
abstracts <- stringr::str_remove_all(abstracts, "</[:alnum:]+>|<[:alnum:]+>")
...
53 of them have no abstract

Now, the title

```{r process-titles, eval = FALSE}
titles <- stringr::str_extract(pub_char_list, "<Title>(\\n|.)+</Title>")
titles <- stringr::str_remove_all(titles, "</[:alnum:]+>|<[:alnum:]+>")
table(is.na(titles))
```

- How many of these don't have a title ? 

All of them have title
